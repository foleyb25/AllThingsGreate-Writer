      ...
      before: {
        '/*': {
          ...
          fn: async function(req, res, next){
            ...
            var loggedInUser = await Writer.findOne({
            ...
            if (loggedInUser.lastSeenAt < now - MS_TO_BUFFER) {
              Writer.updateOne({id: loggedInUser.id})
              .set({ lastSeenAt: now })
              .exec((err)=>{
                if (err) {
            ...
            if (req.method === 'GET') {
              ...
              for (let attrName in Writer.attributes) {
                if (Writer.attributes[attrName].protect) {
              ...
              if (sanitizedUser.password) {
                sails.log.warn('The logged in user record has a `password` property, but it was still there after pruning off all properties that match `protect: true` attributes in the Writer model.  So, just to be safe, removing the `password` property anyway...');
